...
# CLASSIFIER OF NAIVE-BAYER MODEL REGRESSION 
library(e1071)
classifier = naiveBayes(formula = Purchased~.,
                        data = training_set)

# PREDICTION OF OWN CLASSIFIER OF NAIVE-BAYER MODEL REGRESSION 
...
plot(set[, -3],
     main = 'NAIVE BAYER CLASSIFICATION REGRESSION MODEL- SVM (Training set)',
...
y_grid = predict(classifier,  newdata = grid_set)
plot(set[, -3],
     main = 'NAIVE BAYER CLASSIFICATION REGRESSION MODEL - SVM (Testing set)',
     xlab = 'Age', ylab = 'Estimated Salary',
     xlim = range(X1), ylim = range(X2))
contour(X1, X2, matrix(as.numeric(y_grid), length(X1), length(X2)), add = TRUE)
